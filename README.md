...
| Snippets | **Edit** &rarr; **New snippet** adds a passage on **level 0**, a row above the story, for other passages to show inline with `(display: "Code")` &mdash; a recurring description, a status line. The `(display:)` button in the editor inserts one. A passage with no links in or out can be made a snippet from the sidebar&rsquo;s **Advanced** tab, and back again. A snippet sits on no route, so it is never flagged as unreachable or as a dead end; it cannot link, and nothing may link to it (Story stats lists either). Only snippets can be displayed; Play and Publish show the snippet&rsquo;s text in place, and a `(set:)` inside one runs for the passage showing it |
| Changing a code | Rewrites the target half of every inbound link, and every `(display:)` naming it; display text is untouched. A colliding or empty code is blocked, as is one containing link syntax |
| Recoding everything | **Edit** &rarr; **Recode** renames every passage&rsquo;s code at once, from the tree as drawn. **Level and node** gives `3N01` &mdash; the level, then the position within it, left to right. **Node** gives `P04` &mdash; the position in the whole story, top to bottom, left to right within each level. Both take a prefix and a separator, so `3/01`, `L3P01` or a bare `04` are all available. Numbers are padded to at least two digits &mdash; `P04`, `3N01` &mdash; and wider once the story needs it, so codes sort in the order they are drawn and growing past ten does not rewrite every code. Snippets are numbered after the whole story, so the start keeps the first number; in level-and-node codes they are on level `0`. Inbound links and displays follow; a link to a passage that doesn&rsquo;t exist is left alone. One undo step |
...
| Tags | Story-global, reusable from a dropdown, colour-coded from Twine's palette. Recolouring a tag repaints every passage carrying it. A tag sticks around once created, so it stays offerable &mdash; **Tags** &rarr; **Remove** clears out one nothing carries. Wherever tags are listed &mdash; a card's chips, the inspector, the dropdown, the filter bar &mdash; they are grouped by colour in palette order with the uncoloured ones last, matching the stripe along the card's top edge, and sorted by name within a colour |
...
| Story stats | **Story** &rarr; **Stats**, the routes pill, or `Cmd /`. Word count, every ending with the share of routes reaching it, and a draft-health list &mdash; broken links, unreachable passages, dead ends you never marked, links in or to a snippet, and `(display:)` calls that show nothing. Click any row to jump to the passage. Computed when you open it, not as you type |
...
is left out too — and anything left out is named after the save. Snippets travel
inside each passage that displays them rather than on their own, so they are never
reported as left out. It is drawn in the theme
...
| Module | Job |
|---|---|
| `derive` | parse bodies into a graph, resolving link targets against passage codes; unresolved links become phantoms. Snippets stay out of it &mdash; their codes resolve, but a link in or to one is neither an edge nor a phantom |
...
| `xcoord` | lay each component out, then pack them left to right; `LayoutConfig.packing` picks which of the two modules below does the placing |
| `tidy` | bottom-up rigid-subtree placement: parents land on the midpoint of their outermost children — exactly so for a tree, best effort once a passage has two parents. A slide that restores the within-layer order carries the subtree under it, and `recentre` pulls the ancestors above it back toward their midpoints as far as the layer has room for. How it reaches that room is the View menu's **Balanced** / **Aligned** setting: balanced takes only the slack already there, aligned shoves the run ahead and draws wider. A story with no merges is drawn identically either way |
| `straight` | Brandes–Köpf, the View menu's **Straight** setting. Aligns each card with the *median* of its neighbours on the adjacent layer, packs the resulting vertical blocks by longest path, and does it four times — up/down × left/right — taking each card's middle two. No spanning forest and no sweep direction, so neither side of the drawing is favoured; a merged passage sits under a parent rather than off to one side of several, and long links come out straight. It draws wider than **Balanced** though usually narrower than **Aligned**, and it swaps `tidy`'s midpoint rule for a median one, so a lopsided fan moves too — the module header says why no assignment gives both |
...
| `macros` *(in `lib/harlowe/`)* | reads `(set:)`, `(if:)` and `(display:)` as text, so a guarded link can narrow a trail and a recode can move a display with its snippet |
| `run` *(in `lib/harlowe/`)* | the reader's evaluator: a passage's prose, choices and variables, as typed nodes &mdash; a sandbox that feeds nothing above. Stops at an unanswered `(prompt:)` and is re-run with the reader's answers. Walks a `(display:)`ed snippet in place, from bodies the caller hands it |
